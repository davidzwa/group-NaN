%% Introduction
% In this example, an IEEE 802.11ac(TM) waveform is generated using
% doc wlanWaveformGenerator.m
%% Waveform Configuration
% The VHT waveform is specified in 
% doc wlanVHTConfig.m
% Initially config for 40 MHz ChBW, single antenna QPSK rate-1/2 (MCS 1).
...
% In this example a TGac N-LOS with Model-D. Distance RX-TX >= 10 meters.
% doc wlanTGacChannel.m
...
% Typically RCAs use SNR or packet error rate, for rate selection. 
% -> Example estimates SNR of a packet. SNR is compared against threshold.
% For the |threshold| values
% selected for the scenario simulated in this example, a small number of
% packet errors are expected. Assumption correct?

rcaAttack = 1;  % Rensitivity when MCS is increasing, conservative
rcaRelease = 0; % Sensitivity when MCS is decreasing, aggressive
...
numPackets = 5000; % Number of packets transmitted during the simulation 
walkSNR = false; 
...
if walkSNR
    ...
    base = 8;
    max = 30;
    baseSNR = linspace(base,max,numPackets);
...
for numPkt = 1:numPackets 
    ...
    txWave = wlanWaveformGenerator(txPSDU,cfgVHT,'IdleTime',1e-4);
